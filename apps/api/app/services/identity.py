...
# Indian state codes used as the first two characters of a GSTIN.
# Source: GST Council notification; keep in sync with state masters table.
STATE_CODE_TO_NAME: Final[dict[str, str]] = {
...
def get_state_name_from_code(code: str) -> str | None:
    """Return the human-readable state name for a 2-digit GSTIN state code.

    Returns ``None`` for unknown or malformed codes; callers must surface this
    as an anomaly rather than silently default.
    """
    if not isinstance(code, str) or not re.fullmatch(r"[0-9]{2}", code):
        return None
    return STATE_CODE_TO_NAME.get(code)
